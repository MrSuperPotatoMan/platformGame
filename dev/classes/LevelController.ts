...
import { Level,LevelDeconstructed, Vector } from "../levels.js";
import Character from "./Character.js";
import animation from "./animation.js";

export default class LevelController{
    constructor(ctx: CanvasRenderingContext2D,level: LevelDeconstructed,deathCallback:()=>void){
        this.setLevel(level);
        this.#ctx = ctx;
        ...
        animation(this.#animations.fandeIn.bind(this),600)
    }
    setLevel(level:LevelDeconstructed){
        this.#level = {
            width: level.width,
            height: level.height,
    ...
    #animationFrame(time: number){
        ...
        // camera position
        const camera:Vector = [0,0];
        if(this.#character.position[X] > SCREEN_WIDTH / 2){
            if(this.#character.position[X] > this.#level.width - (SCREEN_WIDTH / 2)){
                camera[X] = -1 * (this.#level.width - SCREEN_WIDTH);
                console.log(camera[X])
            }else{
                camera[X] = -1 *(this.#character.position[X] - (SCREEN_WIDTH  /2))
            }
        }
        if(this.#character.position[Y] > SCREEN_HEIGHT / 2){
            if(this.#character.position[Y] > this.#level.height - (SCREEN_HEIGHT / 2)){
                camera[Y] = -1 * (this.#level.height - SCREEN_HEIGHT);
            }else{
                camera[Y] = -1 *(this.#character.position[Y] - (SCREEN_HEIGHT  /2))
            }
        }
        // character
        this.#character.drawCharacter(this.#ctx,delta,camera)
        // blocks
        this.#level.blocks.forEach(el => el.draw(this.#ctx,delta,camera));

        // obstacles // none for now
        this.#level.obstacles.forEach(el => el.draw(this.#ctx,delta,camera));
        this.#animationFrameId = requestAnimationFrame(this.#animationFrame.bind(this))
    }
    #animationFrameId
    #physics(){
        if(this.#keys.a) this.#character.accerelate([-1,0])
        if(this.#keys.d) this.#character.accerelate([1,0])
    ...
    #isDead(){
        ...
        this.#level?.obstacles.forEach(el => {
            if(
                this.#character.position[X] + this.#character.width > el.position[X] &&
    ...
    #animations = {
        fandeIn:(num:number)=>{
            this.#ctx.beginPath();
            this.#ctx.arc(...this.#character.position,num * SCREEN_WIDTH,0,360);
            this.#ctx.rect(SCREEN_WIDTH, 0, -SCREEN_WIDTH, SCREEN_HEIGHT);
            this.#ctx.fill();
            this.#ctx.closePath();
        }
    }
